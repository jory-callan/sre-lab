...
- **Helm 优先** — 全量采用 Helm 部署，无 kustomize / ArgoCD。Helm chart 统一管理，官方 chart 存 tgz，自写 chart 存源码。
...
- **资源配额注释即文档** — `values.yaml` 中 `resources` 用注释标生产推荐值，当前环境只配 `requests.memory` + `limits.memory`（必须成对），CPU 不做限制。
...
│   └── README.md
├── chart/                        特殊保留名 — Helm chart
│                                 官方 chart → tgz（helm-chart-<name>-<version>.tgz）
│                                 自写 chart → 源码（Chart.yaml + templates/）
│                                 gitignore: *.tgz 忽略，!helm-chart-*.tgz 放行
│   ├── helm-chart-xxx-y.y.tgz    官方 chart 命名示例
│   ├── Chart.yaml                自写 chart：Chart.yaml
│   ├── values.yaml               chart 默认 values
│   └── templates/                自写 chart：模板目录
...
resources:
  ...
  # ──────────────────────────────────────────
  # 当前环境最低配置（requests.memory + limits.memory 必配，CPU 不做限制）:
  requests:
    memory: 128Mi
  limits:
    memory: 512Mi
...
| 2 | 实际只配 `requests.memory` + `limits.memory`，**必须成对** |
| 3 | 实际不配 `ephemeral-storage`（兼容性考虑） |
| 4 | 实际不配任何 CPU 字段（requests 和 limits 都不配），CPU 不限制 |
| 5 | 数值按 lab 环境最低能跑的来 |
