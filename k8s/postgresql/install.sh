...
install_instance() {
  ...
  case "$mode" in
    standalone)
      ...
      echo "   集群内: psql -h pg-standalone-rw.postgresql.svc.cluster.local -U postgres -d appdb"
      echo "   密码: pg@czw"
      echo ""
      echo "🔍 查看状态："
    ...
    ha)
      ...
      echo "   集群内(主库): psql -h pg-ha-rw.postgresql.svc.cluster.local -U postgres -d appdb"
      echo "   集群内(只读): psql -h pg-ha-ro.postgresql.svc.cluster.local -U postgres -d appdb"
      echo "   密码: pg@czw"
      echo ""
