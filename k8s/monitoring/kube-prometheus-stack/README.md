...
Prometheus 通过 `release: prometheus` 标签匹配 ServiceMonitor 和 PodMonitor，并配置了 `namespaceSelector: {}`（跨命名空间发现）。其他命名空间的 ServiceMonitor / PodMonitor 添加 `release: prometheus` 标签即可被自动发现。
